...
INITIALIZE:
ldi r16, 0x0C	;load r16 with 0 value
...
out DDRB, r16	;setting DDRB at output
ldi r28, 0x08	;pull up reisitor setup value
out PORTC, r28	;pull up reisitor set
ldi r29, 0x00	;counter set

BEGIN:
cbi PORTB, 3	;set DDRB output to 0
sts TCNT1H, r16	;set timer1 high bits to zero
sts TCNT1L, r16 ;set timer1 low bits to zero
cpi r29, 0x02	;check if two cycles completed
breq LEDOFF		;start LEDOFF cycles if 2 seconds completed
in r30, PINC	;read PINC
andi r30, 0x08	;mask input
cpi r30, 0x08	;button was pressed if equal
breq LEDON		;start LEDON 2 second cycle
rjmp DELAY
...
brlt DELAY			;if less than jump to DELAY
inc r29				;increment 2 second counter
rjmp BEGIN			;back to BEGIN

TOGGLE:
sbi PORTB, 3		;output 1 to PORTB
rjmp DELAY			;back to delay


LEDON:
sbi PORTB, 2		;start 2 sec LED on sequence		
rjmp DELAY

LEDOFF:
cbi PORTB, 2		;stop LED 2 sec cycle
ldi r29, 0x00		;reset counter
rjmp BEGIN